...
const followUser = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: `${currentUser.username} started following ${targetUser.username} 📢` })
  } catch (error) {
...
const unfollowUser = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: `${currentUser.username} Unfollowed ${targetUser.username} 📢` })
  } catch (error) {
    res.status(500).json({ message: `Error following ${targetUser.username} 🔴`, error: error.message })
  }
}
